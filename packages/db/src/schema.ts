...
export const classes = mysqlTable("mu_classes", { id:int("id").autoincrement().primaryKey(), name:text("name").notNull(), teacherId:int("teacher_id").references(()=>users.id), joinCode:varchar("join_code", { length: 20 }).notNull().unique(), status:varchar("status", { length: 20 }).notNull().default("active") });
