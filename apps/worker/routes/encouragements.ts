...
export const handleCreateEncouragement: RouteHandler = async (request, url, env, ctx) => {
  ...
  if (ctx.currentUser.role !== "parent") return json({ error: "Hanya orang tua yang dapat mengirim dukungan." }, 403, {}, "no-store");
  ...
  const [child] = await db.select().from(users).where(and(eq(users.id, childId), eq(users.managedBy, ctx.currentUser.id))).limit(1);
  if (!child) return json({ error: "Profil anak tidak ditemukan." }, 404, {}, "no-store");

  const [created] = await db.insert(encouragements).values({ parentId: ctx.currentUser.id, childId, message }).returning();
...
export const handleListEncouragements: RouteHandler = async (request, url, env, ctx) => {
  ...
  const rows = await db.select({
    ...
    .where(eq(encouragements.childId, ctx.currentUser.id))
...
export const handleMarkEncouragementRead: RouteHandler = async (request, url, env, ctx) => {
  ...
  if (row.childId !== ctx.currentUser.id) return json({ error: "Bukan pesan untukmu." }, 403, {}, "no-store");
