...
export function EditProfileModal({ user, allowRoleEdit, onClose, onSave }: {
  ...
  return <Modal onClose={onClose}>
    <form className="card auth-card" aria-label="Ubah profil" onSubmit={submit}>
      ...
      {allowRoleEdit && <label>Peran<select value={role} onChange={e=>setRole(e.target.value as UserRole)}>{ROLE_OPTIONS_ALL.map(r=><option key={r.value} value={r.value}>{r.label}</option>)}</select></label>}
